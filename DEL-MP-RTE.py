...
def funcMpRouteProcess(strMpRouteConfig):
    ...
    return  
def funcMpRouteDel(dicParameter):
    strResult = "OK"
    # dicParameter를 읽어서 변수에 저장한다.
    id = dicParameter['ID'] 
    # data 리스트에서 id가 일치하는 딕셔너리를 찾아서 삭제한다.
    for item in data:
        if item['id'] == int(id):
            data.remove(item)
            break
    else:
        strResult = f"{id}에 해당하는 데이터가 없습니다."
...
def funcExecMmiRemote(strServerName, strParameter):
    result = ""
    try:
        result = funcExecRemote.funcExecRemote(strServerName,"DEL-MP-RTE.py " + strParameter, "active")
        if "bash" in result:
            print("error: ", result)
        elif len(result) < 1:
            print("error: ", result)
...
def funcServiceRole(dicParameter):
    ...
    #dicParameter ex) -> ID=1
    strResult = funcMpRouteDel(dicParameter)
