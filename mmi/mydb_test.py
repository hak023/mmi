...
class MysqlCommand(MMICommand):
    ...
    def printOutputMessage(self, omsg):
        if omsg.rows is not None :
            ...
            for row in _strrows:
                #print (_row_format % tuple(row[0:columns])).encode('euc-kr')
                print (_row_format % tuple(row[0:columns]))
                #print _row_format % row[0:columns]
    ...
    def findFromTimeToTime(self, timeunit, fromTime=None, toTime=None):
        ...
        if fromTime is None and toTime is None:
            ...
            if timeunit == STAT_TIMEUNIT_5MIN:
                toTime = toTime - datetime.timedelta(minutes=toTime.minute % 5, 
            ...
            if timeunit == STAT_TIMEUNIT_5MIN:
                toTime = fromTime + datetime.timedelta(minutes=5)
                
            elif timeunit == STAT_TIMEUNIT_HOUR:
                toTime = fromTime + datetime.timedelta(hours=1)
                
            elif timeunit == STAT_TIMEUNIT_DAY:
                toTime = fromTime + datetime.timedelta(days=1)
                
            elif timeunit == STAT_TIMEUNIT_WEEK:
                toTime = fromTime + datetime.timedelta(days=7)

            elif timeunit == STAT_TIMEUNIT_MON:
                if fromTime.month == 12:
                    toTime = fromTime.replace(year=fromTime.year+1, month=1)
                else :
                    toTime = fromTime.replace(month=fromTime.month+1)

            elif timeunit == STAT_TIMEUNIT_YEAR:
                toTime = fromTime.replace(year=fromTime.year+1)
