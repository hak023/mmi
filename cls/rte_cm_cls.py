...
class SipRteCommand(IbcfCommand):
    ...
    def printOutputMessage(self, omsg):     
        ...
        if self.isIdxSearch == 0:
           print "\t", "%7s %25s %10s %10s %10s %10s %10s %8s %7s %7s %9s %9s %9s %9s %8s %10s %13s" % ('RTE', 'NAME', 'LOC_ID', 'RMT_ID', 'TRTE', 'TYPE', 'MEDIA', 'OPT_TIME', 'RETRY', 'ACTION', 'SES_TIME', 'GROUP_ID', 'PROTOCOL', 'BUSY_CNT', 'MAX_CNT', 'DEACT_RSP', 'STATUS')
           print "\t", " ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------"
           ...
           for st in sorted(omsg.m_stRte) :
               ...
               if self.rangeCntStart <= self.rtecnt:
                  if self.rangeCntEnd >= self.rtecnt:
                     ...
                     print "\t", "%7d %25s %10d %10d %10d %10s %10s %8d %7d %7s %9d %9d %9s %9d %8d %10s %13s" % (st.m_uiID, self.reprNameShorter(st.m_szDesc, 25, 20), st.m_uiLID, st.m_uiRID, 
                                                                               ...
                                                                               st.m_uiBusyCnt, st.m_nMAXCnt, self.reprReponseIntToStr(st.m_nDeactRsp), self.reprStsIntToStr(st.m_ucStatus)
                                                                               )
