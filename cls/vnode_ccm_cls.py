...
class MSG_EMS_VNODE_DIS_CCM_REQ(IbcfRequestMsg):
...
typedef VNodeInfo_t VNode_Dis_Ccm_Req_t;
    ...
    def __init__(self):
        IbcfRequestMsg.__init__(self, SVC_MSG_MAGIC_COOKIE, self.getSize(), DEF_REQ_MSG_BASE, DEF_STYPE_VNODE_DIS_CCM_REQ)
...
class MSG_EMS_VNODE_DIS_CCM_RSP(IbcfResponseMsg):
...
typedef Dis_VNode_Rsp_t VNode_Dis_Ccm_Rsp_t;
    ...
    def __init__(self):
        IbcfResponseMsg.__init__(self, DEF_STYPE_VNODE_DIS_CCM_RSP)
    ...
    def unpack(self, binary):
        print '>> DEF_STYPE_VNODE_DIS_CCM_RSP unpack'
...
class MSG_EMS_VNODE_CHG_CCM_REQ(VNodeInfo_t_REQ):

    def __init__(self):
        VNodeInfo_t_REQ.__init__(self, SVC_MSG_MAGIC_COOKIE, self.getSize(), DEF_REQ_MSG_BASE, DEF_STYPE_VNODE_CHG_CCM_REQ)

class MSG_EMS_VNODE_CHG_CCM_RSP(VNodeInfo_t_RSP):

    def __init__(self):
        VNodeInfo_t_RSP.__init__(self, DEF_STYPE_VNODE_CHG_CCM_RSP)

###############################################################################################################################################

class VNodeCcmCommand(IbcfCommand):
    ...
    def printInputMessage(self, imsg):
        print ""
        print "\t" "%12s = %d" % ('CCM_INDEX', imsg.m_nID)
        if imsg.m_nMBlock != -1:
    ...
    def printOutputMessage(self, omsg):
        if self.isIdxSearch == 0:
           print "\t", "%9s %20s %30s %9s %30s %9s %7s %7s %7s %7s" % ('CCM_INDEX', 'NAME', 'CM_IP', 'CM_PORT', 'EMP_IP', 'EMP_PORT', 'FBLK', 'MBLK', 'RATE', 'SES')
           print "\t", " -----------------------------------------------------------------------------------------------------------------------------------------------"
                    
           for st in sorted(omsg.m_arrInfo) :
               print "\t", "%9d %20s %30s %9d %30s %9d %7s %7s %7d %7d" % (st.m_nID, self.reprNameShorter(st.m_szVmName, 20, 15), 
           ...
           for st in omsg.m_arrInfo :
               print "\t"
               print "\t" "%12s = %d" % ('EMP_INDEX', st.m_nID)
               print "\t" "%12s = %s" % ('NAME', self.reprName(st.m_szVmName))
               print "\t" "%12s = %s" % ('CM_IP', self.reprName(st.m_szLocalIp))
               print "\t" "%12s = %d" % ('CM_PORT', st.m_nLocalPort)
               print "\t" "%12s = %s" % ('EMP_IP', self.reprName(st.m_szRemoteIp))
               print "\t" "%12s = %d" % ('EMP_PORT', st.m_nRemotePort)
        ...
        if omsg.uiSubType == DEF_STYPE_VNODE_DIS_CCM_RSP:
           print ""
           print "%-10s = %s" % ('CCM_CNT', omsg.m_nNumber)
